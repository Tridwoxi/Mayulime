...
public final class ScrambleSolver extends Solver {
    ...
    public ScrambleSolver(final Consumer<Proposal> listener, final Puzzle puzzle) {
        ...
        this.regionBuffer = new int[puzzle.getFeatures().length];
    }
    ...
    protected void solve() throws KilledException {
        ...
        for (;;) {
            checkAlive();
            final Feature[] scrambled = scramble(best);
            final Feature[] candidate = hillClimbFromFeatures(scrambled);
    ...
    private Feature[] scramble(final Feature[] features) throws KilledException {
        final Feature[] scrambled = features.clone();
        ...
        for (int dy = 0; dy < height; dy += 1) {
            ...
            for (int dx = 0; dx < width; dx += 1) {
                ...
                if (scrambled[cell] == Feature.PLAYER_WALL) {
                    scrambled[cell] = Feature.BLANK;
                    removedWalls += 1;
                }
                if (scrambled[cell] == Feature.BLANK) {
        ...
        for (int placement = 0; placement < localBudget; placement += 1) {
            ...
            scrambled[cell] = Feature.PLAYER_WALL;
