...
public final class DumpSolver extends Solver {
    ...
    private void climbLoop(final Tile[] state, final int[] budgetBox, final int[] scoreBox)
        ...
        for (;;) {
            checkAlive();
            final int[] chokepoints = getChokepoints(state);
            final int[] playerCells = getCellsWhere(state, Tile.PLAYER_WALL);
            IntArrays.shuffleInPlace(chokepoints);
            IntArrays.shuffleInPlace(playerCells);
            if (chokepoints.length > 0) {
                if (placeMoreWalls(state, chokepoints, budgetBox, scoreBox)) {
                    continue;
                }
                if (rearrangeWalls(state, chokepoints, playerCells, scoreBox)) {
                    continue;
                }
            }
            if (dumpOneWall(state, budgetBox, scoreBox)) {
                continue;
            }
            break;
        }
    ...
    private boolean dumpOneWall(final Tile[] state, final int[] budgetBox, final int[] scoreBox)
        throws KilledException {
        if (budgetBox[0] <= 0) {
            return false;
        }
        final int[] shortestPathCells = getShortestPathBlanks(state);
        IntArrays.shuffleInPlace(shortestPathCells);
        for (final int cell : shortestPathCells) {
            checkAlive();
    ...
    private int[] getShortestPathBlanks(final Tile[] state) {
        ...
        return IntArrays.ofRangeWhere(
            0,
            state.length,
            cell -> onPath[cell] && state[cell] == Tile.BLANK
        );
    }
